...
## Pre-requisites
- text editor
- Podman (and optionally podman compose)
- web browser 
...
1. Run Airflow in a Podman container and log in
...
1. Create a new DAG that queries data from a server
    <details>
      ...
      <ul>
      <li>Set up podman compose to also run a database alongside Airflow (recommended to use podman compose and to extend the compose file)</li>
...
## 💡 Hints
### Podman
Get familiar with the Containerfile and build the image.
```bash
podman build -t airflow-ds:latest .
```

Run the image and mount your dags folder, so you will have a quick feedback loop while working on the exercises.
```bash
podman run -it -p 8080:8080 airflow-ds:latest --volume ./dags:/opt/airflow/dags
```
*with compose:*
```bash
podman compose up -d
